require 'spec_helper'
require 'synapse/service_watcher/asg'
require 'logging'

class Synapse::ServiceWatcher::AsgWatcher
  attr_reader   :synapse
  attr_accessor :default_servers, :ec2_client, :asg_client
end
...
describe Synapse::ServiceWatcher::AsgWatcher do
  let(:mock_synapse) { double }
  subject { Synapse::ServiceWatcher::AsgWatcher.new(basic_config, mock_synapse) }
  ...
  describe '#new' do
    ...
    context 'when missing arguments' do
      it 'does not break if aws_region is missing' do
        expect {
          Synapse::ServiceWatcher::AsgWatcher.new(remove_discovery_arg('aws_region'), mock_synapse)
        }.not_to raise_error
      end
      it 'does not break if aws_access_key_id is missing' do
        expect {
          Synapse::ServiceWatcher::AsgWatcher.new(remove_discovery_arg('aws_access_key_id'), mock_synapse)
        }.not_to raise_error
      end
      it 'does not break if aws_secret_access_key is missing' do
        expect {
          Synapse::ServiceWatcher::AsgWatcher.new(remove_discovery_arg('aws_secret_access_key'), mock_synapse)
        }.not_to raise_error
      end
      it 'complains if server_port_override is missing' do
        expect {
          Synapse::ServiceWatcher::AsgWatcher.new(remove_haproxy_arg('server_port_override'), mock_synapse)
    ...
    context 'invalid data' do
      it 'complains if the haproxy server_port_override is not a number' do
          expect {
            Synapse::ServiceWatcher::AsgWatcher.new(munge_haproxy_arg('server_port_override', '80deadbeef'), mock_synapse)
  ...
  context "instance discovery" do
    ...
    context 'returned backend data structure' do
      before do
        allow(subject).to receive(:instances_in_asg).and_return([instance1, instance2])
      end
    ...
    context 'returned instance fields' do
      before do
        allow(subject).to receive(:instances_in_asg).and_return([instance1])
      end
